<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <!--==================== UNICONS ====================-->
    <link rel="stylesheet" href="https://unicons.iconscout.com/release/v4.0.0/css/line.css">
    <link rel="shortcut icon" type="image/x-icon" href="{{ asset('assets/img/logos.png') }}" />
    <!--==================== SWIPER CSS ====================-->


    <!--==================== CSS ====================-->
    <link rel="stylesheet" href="{{ asset('assets/css/main.css') }}">
    <link rel="stylesheet" href="{{ asset('assets/css/media.css') }}">

    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@700&display=swap" rel="stylesheet">
    <!-- {{-- SCROLL REVEAL LIBRARY --}} -->
    <script src="https://unpkg.com/scrollreveal"></script>
    <title>Open to Work and New Project </title>
</head>
<style>
</style>
<main>

    <body>
        <header class="topbar">
            <nav class="navbar">
                <div class="logo">
                    <h3>Hi,</h3>
                </div>
                <ul class="nav-links">
                    <li><a href="#about-section_2">Services</a></li>
                    <li><a href="#my-project">My Work</a></li>
                    <li><a href="#contact">Contact</a></li>

                </ul>
                <a href="mailto:contact.adimertap@gmail.com?subject=New%20Project%20Inquiry&body=Hello,%0D%0A%0D%0AI%20am%20interested%20in%20starting%20a%20new%20project%20and%20would%20like%20to%20discuss%20it%20with%20you.%0D%0A%0D%0AThank%20you,%0D%0A%5BYour%20Name%5D" class="start-btn">Start New Project</a>
            </nav>
        </header>

        <section class="hero">
            <div class="container">
                <div class="inner-content primary">
                    <article class="article-content cavalier">
                        <h1>I'am Adim, Full Stack Developer.</h1>
                        {{-- <p>I have worked on a variety of projects, ranging from small business websites to
                            large-scale
                            web applications. on various projects, I specialize in building dynamic web applications, I
                            am committed to delivering high-quality work that exceeds my clients' expectations. Scroll
                            down to see My Portofolio Project</p> --}}
                        {{-- <p>With a deep commitment to quality and innovation, I specialize in transforming ideas into
                            sleek, high-performing digital solutions that deliver real results. Whether you need a
                            streamlined website, a complex web application, or robust API integrations, my expertise
                            spans the full development process—from design to deployment.
                        </p> --}}
                        {{-- <p>
                            <b>Open to Work in Software Development! </b> I’m passionate about transforming ideas into high-performing digital solutions. From sleek websites to complex applications and API integrations, I bring expertise across the full development process to deliver impactful results.
                        </p> --}}
                        <p>
                            <b>I’m open to work and excited to take on new projects!</b> From small business sites to large-scale applications, I deliver high-performance, custom web solutions with full-stack expertise. With a focus on quality and innovation, let’s build something that drives your success.
                        </p>
                        {{-- <p>
                            I'm open for new projects, specializing in application and software development that drives real impact. Whether it’s a custom app, a streamlined web solution, or robust software integrations, I deliver quality results that fuel growth.
                        </p> --}}
                        <div class="social-wrapper">
                            <a href="https://www.linkedin.com/in/i-putu-adi-merta-pratama/" class="icon linkedin">
                                <div class="tooltip">LinkedIn</div>
                                <span><i class="uil uil-linkedin"></i></span>
                            </a>
                            <a href="https://www.instagram.com/adiimerta/" class="icon instagram">
                                <div class="tooltip">Instagram</div>
                                <span><i class="uil uil-instagram"></i></span>
                            </a>
                            <a href="https://github.com/adimertap" class="icon github">
                                <div class="tooltip">Github</div>
                                <span><i class="uil uil-github"></i></span>
                            </a>
                            <a href="https://github.com/adimertap" class="icon fiverr">
                                <div class="tooltip">Fiver</div>
                                <span>
                                    <svg class="mt-2" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em"
                                        viewBox="-2.5 -2 24 24">
                                        <g fill="currentColor">
                                            <path
                                                d="M16.25 16.25v-10h-10v-.625c0-1.034.841-1.875 1.875-1.875H10V0H8.125A5.63 5.63 0 0 0 2.5 5.625v.625H0V10h2.5v6.25H0V20h8.75v-3.75h-2.5V10h6.285v6.25H10V20h8.75v-3.75z" />
                                            <circle cx="14.375" cy="1.875" r="1.875" />
                                        </g>
                                    </svg>
                                </span>
                            </a>
                        </div>

                    </article>
                    <figure class="patern-figure">
                        <svg xmlns="http://www.w3.org/2000/svg" width="935" height="701" viewBox="0 0 935 701"
                            id="avatar-shapes" data-shape="true">
                            <path fill="#ccf381"
                                d="M112.476 1.58a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM131.444 1.58a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 1.58a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM169.38 1.58a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM188.348 1.58a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM207.315 1.58a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM226.283 1.58a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM245.252 1.58a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM264.22 1.58a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM283.188 1.58a1.5 1.5 0 11-3.002 0 1.5 1.5 0 013.002 0zM302.155 1.58a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 1.58a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM112.476 20.804a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM131.444 20.804a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM150.412 20.804a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM169.38 20.804a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM188.348 20.804a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM207.315 20.804a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM226.283 20.804a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM245.252 20.804a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM264.22 20.804a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM283.188 20.804a1.5 1.5 0 11-3.002-.001 1.5 1.5 0 013.002 0zM302.155 20.804a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM321.123 20.804a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM112.476 40.026a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM131.444 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM169.38 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM188.348 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM207.315 40.026a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM226.283 40.026a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM245.252 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM264.22 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM283.188 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM302.155 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 40.026a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM112.476 59.25a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM131.444 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM169.38 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM188.348 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM207.315 59.25a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM226.283 59.25a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM245.252 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM264.22 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM283.188 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM302.155 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 59.25a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM112.476 78.473a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM131.444 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM169.38 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM188.348 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM207.315 78.473a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM226.283 78.473a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM245.252 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM264.22 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM283.188 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM302.155 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 78.473a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM112.476 97.696a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM131.444 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM169.38 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM188.348 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM207.315 97.696a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM226.283 97.696a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM245.252 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM264.22 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM283.188 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM302.155 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 97.696a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM112.476 116.92a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM131.444 116.92a1.5 1.5 0 11-3-.002 1.5 1.5 0 013 .001zM150.412 116.92a1.5 1.5 0 11-3.001-.002 1.5 1.5 0 013.001.001zM169.38 116.92a1.5 1.5 0 11-3.001-.002 1.5 1.5 0 013 .001zM188.348 116.92a1.5 1.5 0 11-3.001-.002 1.5 1.5 0 013 .001zM207.315 116.92a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM226.283 116.92a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM245.252 116.92a1.5 1.5 0 11-3.001-.002 1.5 1.5 0 013.001.001zM264.22 116.92a1.5 1.5 0 11-3.001-.002 1.5 1.5 0 013 .001zM283.188 116.92a1.5 1.5 0 11-3.002-.002 1.5 1.5 0 013.002.001zM302.155 116.92a1.5 1.5 0 11-3-.002 1.5 1.5 0 013 .001zM321.123 116.92a1.5 1.5 0 11-3.001-.002 1.5 1.5 0 013.001.001zM112.476 136.142a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM131.444 136.142a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 136.142a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM169.38 136.142a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM188.348 136.142a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM207.315 136.142a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM226.283 136.142a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM245.252 136.142a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM264.22 136.142a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM283.188 136.142a1.5 1.5 0 11-3.002 0 1.5 1.5 0 013.002 0zM302.155 136.142a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 136.142a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM112.476 155.365a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM131.444 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM169.38 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM188.348 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM207.315 155.365a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM226.283 155.365a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM245.252 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM264.22 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM283.188 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM302.155 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 155.365a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM112.476 174.588a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM131.444 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM169.38 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM188.348 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM207.315 174.588a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM226.283 174.588a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM245.252 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM264.22 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM283.188 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM302.155 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 174.588a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM112.476 193.811a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM131.444 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM150.412 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM169.38 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM188.348 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM207.315 193.811a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM226.283 193.811a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM245.252 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM264.22 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM283.188 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM302.155 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM321.123 193.811a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z">
                            </path>
                            <path fill="#4831d4"
                                d="M692.477 418.551a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM711.444 418.551a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM730.412 418.551a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM749.38 418.551a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM768.348 418.551a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM787.315 418.551a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 418.551a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 418.551a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM844.22 418.551a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM863.188 418.551a1.5 1.5 0 11-3.002 0 1.5 1.5 0 013.001 0zM882.155 418.551a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM901.123 418.551a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM692.477 437.774a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM711.444 437.774a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 .001zM730.412 437.774a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001.001zM749.38 437.774a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM768.348 437.774a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM787.315 437.774a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 437.774a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 437.774a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001.001zM844.22 437.774a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM863.188 437.774a1.5 1.5 0 11-3.002-.001 1.5 1.5 0 013.001.001zM882.155 437.774a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 .001zM901.123 437.774a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001.001zM692.477 456.998a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM711.444 456.998a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM730.412 456.998a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM749.38 456.998a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM768.348 456.998a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM787.315 456.998a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 456.998a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 456.998a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM844.22 456.998a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM863.188 456.998a1.5 1.5 0 11-3.002-.001 1.5 1.5 0 013.001 0zM882.155 456.998a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM901.123 456.998a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM692.477 476.22a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM711.444 476.22a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM730.412 476.22a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM749.38 476.22a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM768.348 476.22a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM787.315 476.22a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 476.22a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 476.22a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM844.22 476.22a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013 0zM863.188 476.22a1.5 1.5 0 11-3.002 0 1.5 1.5 0 013.001 0zM882.155 476.22a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM901.123 476.22a1.5 1.5 0 11-3.001 0 1.5 1.5 0 013.001 0zM692.477 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM711.444 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM730.412 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM749.38 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM768.348 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM787.315 495.443a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM806.283 495.443a1.5 1.5 0 11-2.998.001 1.5 1.5 0 012.998-.001zM825.252 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM844.22 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM863.188 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM882.155 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM901.123 495.443a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM692.477 514.667a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM711.444 514.667a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM730.412 514.667a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM749.38 514.667a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM768.348 514.667a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM787.315 514.667a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 514.667a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 514.667a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM844.22 514.667a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM863.188 514.667a1.5 1.5 0 11-3.002-.001 1.5 1.5 0 013.001 0zM882.155 514.667a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM901.123 514.667a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM692.477 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM711.444 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM730.412 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM749.38 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM768.348 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM787.315 533.89a1.5 1.5 0 11-2.998 0 1.5 1.5 0 012.998 0zM806.283 533.89a1.5 1.5 0 11-2.998 0 1.5 1.5 0 012.998 0zM825.252 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM844.22 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM863.188 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM882.155 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM901.123 533.89a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM692.477 553.113a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM711.444 553.113a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 .001zM730.412 553.113a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001.001zM749.38 553.113a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM768.348 553.113a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM787.315 553.113a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 553.113a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 553.113a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001.001zM844.22 553.113a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 .001zM863.188 553.113a1.5 1.5 0 11-3.002-.001 1.5 1.5 0 013.001.001zM882.155 553.113a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 .001zM901.123 553.113a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001.001zM692.477 572.336a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM711.444 572.336a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM730.412 572.336a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM749.38 572.336a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM768.348 572.336a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM787.315 572.336a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 572.336a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 572.336a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM844.22 572.336a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM863.188 572.336a1.5 1.5 0 11-3.002-.001 1.5 1.5 0 013.001 0zM882.155 572.336a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM901.123 572.336a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM692.477 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM711.444 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM730.412 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM749.38 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM768.348 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM787.315 591.558a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM806.283 591.558a1.5 1.5 0 11-2.998.002 1.5 1.5 0 012.998-.002zM825.252 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM844.22 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM863.188 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM882.155 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM901.123 591.558a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM692.477 610.782a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM711.444 610.782a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM730.412 610.782a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM749.38 610.782a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM768.348 610.782a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM787.315 610.782a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM806.283 610.782a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM825.252 610.782a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0zM844.22 610.782a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013 0zM863.188 610.782a1.5 1.5 0 11-3.002-.001 1.5 1.5 0 013.001 0zM882.155 610.782a1.5 1.5 0 11-3-.001 1.5 1.5 0 013 0zM901.123 610.782a1.5 1.5 0 11-3.001-.001 1.5 1.5 0 013.001 0z">
                            </path>
                        </svg>
                        <span role="img" class="handsome"></span>
                    </figure>
                </div>
                <div class="inner-content bottom">
                    <ul class="highlights" id="highlights-bottom">
                        <li><b>Contact me to discuss</b>—online or in person (Jakarta area)
                        </li>
                        <li><a href="mailto:contact.adimertap@gmail.com?subject=New%20Project%20Inquiry&body=Hello,%0D%0A%0D%0AI%20am%20interested%20in%20starting%20a%20new%20project%20and%20would%20like%20to%20discuss%20it%20with%20you.%0D%0A%0D%0AThank%20you,%0D%0A%5BYour%20Name%5D"
                            class="btn btn-master btn-secondary me-3"
                                style="height:45px!important; font-size:15px!important; font-weight: 600!important">
                                Contact Me
                            </a></li>
                        {{-- <li>Let’s connect to create something exceptional for your brand. </li> --}}
                    </ul>
                </div>
            </div>
        </section>


        <header class="header_2">
            <div class="moving-text_2">
                <span class="text-item_2">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2">GET STARTED</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET STARTED</span><span class="star_2"></span>

                <!-- Repeat enough times to ensure continuous scrolling -->
            </div>
        </header>
        <section id="about-section_2" class="about-section_2">
            <div class="about-text_2 flip-container">
                <!-- Front Side -->
                <div class="flip-card-front">
                    <h3 class="about-heading_2">SERVICES</h3>
                    <h1 class="main-title_2">
                        Crafting High-Impact <span class="highlight_2">Digital Solutions</span> for Your Industry
                    </h1>
                    <p class="about-description_2">
                        Every project I take on is meticulously crafted to exceed expectations, integrating design and
                        technology to deliver a smooth and impactful user experience.
                    </p>
                    <a href="#" class="learn-more-link_2" onclick="flipCard(event)">(Click Me) My Tech Stack →</a>
                    <div class="image-wrapper">
                        <img src="{{ asset('assets/img/3d/torus.png') }}" alt="3D Image" class="top-left-image-service">
                    </div>
                </div>

                <!-- Back Side -->
                <div class="flip-card-back">
                    <h3 class="about-heading_2">TECH STACK</h3>
                    <h1 class="main-title_2">
                        <span class="highlight_2">High-Performance</span> Tech Stack
                    </h1>
                    <p class="about-description_2" style="margin-bottom: 0px!important;margin-top: 40px!important">
                        <b>Website Development:</b> React JS, Typescript, VB.NET, Laravel, PHP,
                    </p>
                    <p class="about-description_2" style="margin-bottom: 0px!important">
                        <b>API Development:</b> Express JS, Typescript, PHP
                    </p>
                    <p class="about-description_2" style="margin-bottom: 20px!important">
                        <b>Mobile Development:</b> Expo React Native Typescript
                    </p>
                    <p class="about-description_2">
                        Flexible to work with your preferred technologies to meet your project’s unique requirements.

                    </p>
                    <a href="#" class="learn-more-link_2" onclick="flipCard(event)">Services →</a>
                    <div class="image-wrapper">
                        <img src="{{ asset('assets/img/3d/half-torus.png') }}" alt="3D Image" class="top-left-image-service">
                    </div>
                </div>
            </div>

            <div class="testimonial-container_2">
                <div class="testimonial_2">
                    <div class="profile-picture_2">
                        <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="15"
                            height="15">
                        {{-- <img src="path_to_image" alt="Muhammad Sultan"> --}}
                    </div>
                    <div class="testimonial-text_2">
                        <h4 class="profile-name_2">Project Management and Web Development</h4>
                        <p class="profile-role_2">2 Years Experienced Full Stack Developer</p>
                        <p class="testimonial-quote_2">
                            {{-- "Building intuitive, modern, and responsive web experiences that are not only visually
                            appealing but also efficient and fast. With a full-stack approach, each solution is designed
                            to deliver exceptional user experiences and high performance." --}}
                            {{-- "Managing design and database projects to ensure seamless execution. Crafting
                            responsive, high-performance web solutions that exceed client expectations." --}}
                            "Experienced in managing and executing web projects from start to finish. I build
                            responsive, high-performance websites that are not only visually appealing but also
                            optimized for functionality and speed."
                        </p>
                    </div>
                </div>
                <hr class="divider_2">
                <div class="testimonial_2">
                    <div class="profile-picture_2">
                        <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="15"
                            height="15">
                    </div>
                    <div class="testimonial-text_2">
                        <h4 class="profile-name_2">API Development</h4>
                        <p class="profile-role_2">2 Years Experienced API Solutions Developer</p>
                        <p class="testimonial-quote_2">
                            {{-- "Building high-performance, scalable RESTful APIs with secure, efficient integrations
                            to support robust applications." --}}
                            "Expert in creating high-performance, scalable RESTful APIs with secure, efficient
                            integrations that support powerful applications. Each API is crafted to adapt seamlessly as
                            your needs grow, ensuring lasting reliability and impact."
                            {{-- "Building high-performance, scalable RESTful APIs with secure, efficient integrations
                            to support robust applications. Each API is crafted to adapt seamlessly as your needs grow,
                            ensuring lasting reliability and impact." --}}
                        </p>
                    </div>
                </div>
                <hr class="divider_2">
            </div>
        </section>

        <header class="header_2">
            <div class="moving-text_2">
                <span class="text-item_2">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2">PROJECT SUCCESS</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">PROJECT SUCCESS</span><span class="star_2"></span>
            </div>
        </header>



        <section id="my-project" class="my-project">
            {{-- <div class="container"> --}}
                <div class="row">
                    <div class="col-lg-6 col-sm-12 my-project-info">
                        <div class="my-project-top">
                            <h2 class="section_title">Project <span class="highlight_2">Completed</span></h2>
                            <p class="my-project-h2-p">Some projects I've worked on and have finished, project
                                details to see project details.</p>
                        </div>
                    </div>
                    <div class="col-lg-12 col-sm-12 my-project-content">
                        <div class="my-project-bottom">
                            <div class="card">
                                <div class="card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">

                                    <span>E-Learning Bootcamp</span>
                                </div>
                                <p>Web-based application that supports members to join online training or bootcamp.
                                    This application provides many courses to increase knowledge and hone hard
                                    skills of members, offers job vacancies, and integrates a payment gateway API for
                                    seamless and secure transactions.</p>
                                <a href="{{ route('bootcamp') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class="card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>E-Bengkel</span>
                                </div>
                                <p>E-Bengkel is a comprehensive web application designed to streamline workshop management through nine seamlessly integrated modules. These include Front Office, Point of Sales, Service, Inventory & Purchasing, Staffing, Payroll and Accounting & Finance, all working together to optimize every aspect of your operations.</p>
                                <a href="{{ route('bengkel') }}">Click for Detail</a>
                                <i class="details text-center" style="margin-top: 80px">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Payroll Management</span>
                                </div>

                                <p>The Payroll web app simplifies monthly management for treasurers, providing a comprehensive tool to oversee employee salaries at KPP Pratama Gianyar Bali. This application allows treasurers to view detailed monthly reports, which include all components of payroll, ensuring efficient and accurate salary management.</p>
                                <a href="{{ route('payroll') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>HRIS </span>
                                </div>
                                <p> Human Resources Information System is a powerful web-based solution designed to streamline personnel management at PT. XYZ With 14 robust features—covering everything from employee onboarding, scheduling, and attendance to promotions, self-assessment, and more—and 7 distinct user roles, this application is built to elevate HR operations.</p>
                                <a href="{{ route('hrd') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Money Changer</span>
                                </div>

                                <p>The Money Changer Cashier is a web-based solution for efficient cashier transactions and accounting reports at PT. Riastavalasindo. Key features include daily capital management, transaction reporting, automated journal entries, and easy download/print options in Excel and PDF, streamlining financial operations seamlessly.</p>
                                <a href="{{ route('kasir') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Inventory & Sales Gas Supplier</span>
                                </div>
                                <p>This web application streamlines supply and sales management for gas and aqua products at PT Suksesberkahbertumbuh. With 10 features and 3 user roles, it ensures accurate stock tracking and standardized sales reporting, solving key operational challenges.</p>
                                <a href="{{ route('inventory') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Inventory Management</span>
                                </div>
                                <p>The Inventory Management System for one of the Tax Service Offices (Kantor Pelayanan Pajak) in Bali is a comprehensive solution designed to address challenges such as inefficient record-keeping and untracked or missing assets. This system ensures precise asset tracking and streamlined operations, thereby enhancing overall efficiency and accountability.</p>
                                <a href="{{ route('inventory-kpp') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>E-Learning IC Education</span>
                                </div>
                                <p>
                                    IC Education E-Learning System redefines professional training with premium Brevet courses, seminars, and USKP preparation, offering seamless transactions via Midtrans API for unmatched accessibility and excellence. Designed for both individuals and organizations, it ensures a superior learning experience tailored to diverse needs.
                                </p>
                                <a href="{{ route('ic') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Laundry dan Money Changer Cashier</span>
                                </div>
                                <p>
                                    The Laundry Management System by PT. Riastavalasindo streamlines transaction logging, reporting, and master data organization, ensuring efficient and accurate operations for laundry businesses.
                                </p>
                                <a href="{{ route('laundry') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>
                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Land or Building Rental Registration Admin Website</span>
                                </div>
                                <p>
                                    An efficient platform for managing rental data, designed for a local government in Bali to streamline tenant tracking, ensure accurate records, and enhance administrative accountability.
                                </p>
                                <a href="{{ route('sewa') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>
                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Fun Project Movie Apps</span>
                                </div>
                                <p>This application is a simple application to <b>watch movies</b> like Netflix or
                                    DisneyPlus. The application is built with the laravel framework with reactjs support
                                    with user improvements.</p>
                                <a href="{{ route('movie') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>

                            </div>
                            <div class="card">
                                <div class=" card-heading">
                                    <img alt="development" src="{{ asset('assets/img/sparkle-black2.png') }}" width="25"
                                        height="25">
                                    <span>Fun Project Travel Apps</span>
                                </div>
                                <p>This application is a simple application for practice and learning reactjs. The
                                    application is built with reactjs and slicing from user interface design on figma.
                                </p>
                                <a href="{{ route('travel') }}">Click for Detail</a>
                                <i class="details text-center">Click to See Detail</i>
                            </div>

                        </div>
                        <p class="mt-5 mb-5 text-center btn-project">
                            <a href="#" class="btn btn-master btn-secondary me-3">
                                Ready for Next Project!
                            </a>
                        </p>
                    </div>
                    {{--
                </div> --}}

            </div>
        </section>
        <header class="header_3">
            <hr class="divider_2">
            <i>"Good software design is about satisfying a client’s requirements in a way that’s maintainable, flexible, and efficient. Developers must think beyond the code and see the broader goals—solutions that solve problems, add value, and align with clients' visions." -Steve McConnell's</i>
            <p></p>
            <hr class="divider_2">
            <i>In software development, our true job is solving problems for our clients, building systems that serve their goals. Code is just a tool; what matters is understanding the problem, defining the solution, and being relentlessly focused on client success. -Andrew Hunt and David Thomas </i>
            <hr class="divider_2">
        </header>

        <header class="header_2">
            <div class="moving-text_2">
                <span class="text-item_2" style="color: #ccf381">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2" style="color: #ccf381">GET IN TOUCH</span><span class="star_2"></span>
                <span class="text-item_2">GET IN TOUCH</span><span class="star_2"></span>

                <!-- Repeat enough times to ensure continuous scrolling -->
            </div>
        </header>


        <section id="contact" class="contact">
            <div class="container">
                <div class="image-wrapper">
                    <!-- Add your 3D image at the top-right (first marker) -->
                    <img src="{{ asset('assets/img/3d/emojistar.png') }}" alt="3D Image" class="top-right-image">
                </div>
                <div class="grids">
                    <div class="contact-info">
                        <h1 class="main-title_2">
                            Start a <span class="highlight_2">Conversation</span>
                        </h1>
                        {{-- <h1 class="section_title">Get in Touch</h1> --}}
                        <p class="my-project-h2-p">
                            I’m always open to new projects and excited to discuss how I can support your vision. Feel free to reach out anytime to explore how we can collaborate to bring your web development ideas to life. I look forward to the possibility of working together. Thank you!
                        </p>
                        <a href="mailto:contact.adimertap@gmail.com" class="mail">
                            Email: contact.adimertap@gmail.com <i class="uil uil-arrow-right"></i>
                        </a>
                        <p class="mail">
                            WhatsApp: (+62) 81246602400
                        </p>
                    </div>
                    <div class="image-wrapper">
                        <!-- Add your 3D image at the bottom-left (second marker) -->
                        <img src="{{ asset('assets/img/3d/torus.png') }}" alt="3D Image" class="bottom-left-image">
                    </div>
                    {{-- <form action="" class="contact-form">
                        <h3>Send me a message</h3>
                        <input type="text" class="form-input" placeholder="Your Name" required>
                        <input type="email" class="form-input" placeholder="Your Email" required>
                        <textarea placeholder="Key Message" class="form-input" required></textarea>
                        <a href="#" class="btn btn-master btn-secondary">
                            Send
                        </a>

                    </form> --}}

                </div>
            </div>
        </section>


        <section class="section-row lets-talk lets-talk-2019">
            <div class="container">
                <div class="text-content-talk">
                    <h4>Need Website Product?</h4>
                    <hr>
                    <p>Interested in creating <b>a website product</b> for your company? Let's Talk!. My team will find
                        a solution to your company's problems.</p>
                </div>
                <a href="mailto:contact.adimertap@gmail.com?subject=New%20Project%20Inquiry&body=Hello,%0D%0A%0D%0AI%20am%20interested%20in%20starting%20a%20new%20project%20and%20would%20like%20to%20discuss%20it%20with%20you.%0D%0A%0D%0AThank%20you,%0D%0A%5BYour%20Name%5D"
                class="btn-below btn-default btn-foot"
                    data-toggle="form-modal">Let's Talk <i class="fa fa-chevron-right"></i></a>
            </div>

        </section>
</main>

<!--==================== MAIN JS ====================-->
<script src="{{ asset('assets/js/main.js') }}"></script>
<script>
    document.querySelectorAll('.my-project-bottom .card').forEach(card => {
        card.addEventListener('click', function() {
            const link = this.querySelector('a');
            if (link && link.href) {
                window.location.href = link.href;
            }
        });
    });
    window.onscroll = function() {stickyNavbar()};

    var navbar = document.querySelector(".topbar");
    var sticky = navbar.offsetTop;

    function stickyNavbar() {
        if (window.pageYOffset > sticky) {
            navbar.classList.add("sticky");
        } else {
            navbar.classList.remove("sticky");
        }
    }
    function flipCard(event) {
    event.preventDefault();
    const flipContainer = document.querySelector('.flip-container');
    flipContainer.classList.toggle('flip');
}
</script>
</body>

</html>
